...
* Programming
 ...
 #+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (define-key my/eglot-map (kbd "d") #'xref-find-definitions)
    (define-key my/eglot-map (kbd "f") #'xref-find-references)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Cache for Flymake mode line
  (defvar-local my/flymake-mode-line-cache nil
    "Cached Flymake mode line string.")

  (defvar-local my/flymake-last-diag-count nil
    "Last diagnostic count for cache invalidation.")

  (defun my/flymake-mode-line ()
    "Return a cached string showing Flymake diagnostics and backend."
    (when (bound-and-true-p flymake-mode)
      (let ((current-count (length (flymake-diagnostics))))
        (unless (equal current-count my/flymake-last-diag-count)
          (setq my/flymake-last-diag-count current-count)
          (setq my/flymake-mode-line-cache nil)))
      (or my/flymake-mode-line-cache
          (my/flymake-compute-mode-line))))

  (defun my/flymake-compute-mode-line ()
    "Compute Flymake mode line string."
    (let ((error-count 0)
          (warning-count 0)
          (note-count 0)
          (backend-name nil)
          (diags (flymake-diagnostics)))

      ;; Count diagnostics
      (dolist (diag diags)
        (let ((type (flymake-diagnostic-type diag)))
          (cond
           ((memq type '(eglot-error :error))
            (setq error-count (1+ error-count)))
           ((memq type '(eglot-warning :warning))
            (setq warning-count (1+ warning-count)))
           ((memq type '(eglot-note :note))
      ...
      (when (and (featurep 'eglot) (eglot-current-server))
        (condition-case nil
            (let* ((server (eglot-current-server))
                   ...
                   (cmd (when proc (process-command proc))))
              (when (and cmd (car cmd))
                (setq backend-name (file-name-nondirectory (car cmd)))))
          (error
           (setq backend-name "lsp"))))

      ;; Build string
      (setq my/flymake-mode-line-cache
            (concat
             (when backend-name
               (propertize (format " [%s]" backend-name) 'face 'font-lock-comment-face))
             (cond
              ((> error-count 0)
               (concat
                (propertize (format " ✖%d" error-count) 'face 'error)
                (when (> warning-count 0)
                  (propertize (format " ⚠%d" warning-count) 'face 'warning))))
              ((> warning-count 0)
               (propertize (format " ⚠%d" warning-count) 'face 'warning))
              (backend-name
               (propertize " ✓" 'face 'success)))))

      my/flymake-mode-line-cache))

  ;; Hook to invalidate cache when Flymake updates
  (defun my/flymake-invalidate-cache (&rest _)
    "Invalidate Flymake mode line cache."
    (setq my/flymake-mode-line-cache nil)
    (force-mode-line-update))

  (add-hook 'flymake-after-diagnostics-hook
            #'my/flymake-invalidate-cache)
